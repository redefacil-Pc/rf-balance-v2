...
interface Props { colaboradores: Collaborator[]; podeVerPii: boolean; podeEditar: boolean; onEditar: (item: Collaborator) => void; onInativar: (item: Collaborator) => void; onFuncoes: (item: Collaborator) => void }

export function CollaboratorTable({ colaboradores, podeVerPii, podeEditar, onEditar, onInativar, onFuncoes }: Props) {
  return <Table.ScrollContainer minWidth={780}><Table striped highlightOnHover verticalSpacing="sm">
    ...
    <Table.Tbody>{colaboradores.map((item) => <Table.Tr key={item.id}>
      ...
      {podeEditar && <Table.Td><Group gap={4} wrap="nowrap"><Tooltip label="Editar cadastro"><ActionIcon variant="subtle" aria-label={`Editar ${item.full_name}`} onClick={() => onEditar(item)}><IconPencil size={16} /></ActionIcon></Tooltip><Tooltip label="Inativar colaborador"><ActionIcon variant="subtle" color="red" disabled={!item.is_active} aria-label={`Inativar ${item.full_name}`} onClick={() => onInativar(item)}><IconTrash size={16} /></ActionIcon></Tooltip></Group></Table.Td>}
