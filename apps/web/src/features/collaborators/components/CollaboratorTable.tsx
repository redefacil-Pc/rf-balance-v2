...
import { IconBuildingBank, IconPencil, IconPlayerPlay, IconTrash, IconUserCog } from '@tabler/icons-react';
...
interface Props {
  ...
  podeVerPii: boolean;
  podeEditar: boolean;
  onEditar: (item: Collaborator) => void;
  onInativar: (item: Collaborator) => void;
  onFuncoes: (item: Collaborator) => void;
  onContas: (item: Collaborator) => void;
  onAcesso: (item: Collaborator) => void;
}

export function CollaboratorTable({ colaboradores, podeVerPii, podeEditar, onEditar, onInativar, onFuncoes, onContas, onAcesso }: Props) {
  return <Table.ScrollContainer minWidth={780}><Table striped highlightOnHover verticalSpacing="sm">
    ...
    <Table.Tbody>{colaboradores.map((item) => <Table.Tr key={item.id}>
      ...
      <Table.Td>{item.user_id ? <Badge size="sm" variant="light" color={item.user_is_active ? 'blue' : 'yellow'}>{item.user_is_active ? 'Com acesso' : 'Conta inativa'}</Badge> : <Text size="xs" c="dimmed">sem conta</Text>}</Table.Td>
      <Table.Td><Badge size="sm" color={item.is_active ? 'positivo' : 'gray'} variant="light">{item.is_active ? 'Ativo' : 'Inativo'}</Badge></Table.Td>
      {podeEditar && <Table.Td><Group gap={4} wrap="nowrap"><Tooltip label="Editar cadastro"><ActionIcon variant="subtle" aria-label={`Editar ${item.full_name}`} onClick={() => onEditar(item)}><IconPencil size={16} /></ActionIcon></Tooltip><Tooltip label="Conta de acesso"><ActionIcon variant="subtle" aria-label={`Acesso de ${item.full_name}`} onClick={() => onAcesso(item)}><IconUserCog size={16} /></ActionIcon></Tooltip>{podeVerPii && <Tooltip label="Contas bancárias"><ActionIcon variant="subtle" aria-label={`Contas de ${item.full_name}`} onClick={() => onContas(item)}><IconBuildingBank size={16} /></ActionIcon></Tooltip>}<Tooltip label={item.is_active ? 'Inativar colaborador' : 'Reativar colaborador'}><ActionIcon variant="subtle" color={item.is_active ? 'red' : 'positivo'} aria-label={`${item.is_active ? 'Inativar' : 'Reativar'} ${item.full_name}`} onClick={() => onInativar(item)}>{item.is_active ? <IconTrash size={16} /> : <IconPlayerPlay size={16} />}</ActionIcon></Tooltip></Group></Table.Td>}
